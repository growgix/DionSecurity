...
export const DataStoreProvider = ({ children }) => {
  ...
  // Synchronize data from PHP + MySQL backend when authenticated
  useEffect(() => {
    if (!currentUser) {
    ...
    async function syncFromBackend() {
      try {
        ...
        if (health && health.status === 'online') {
          ...
          console.log('[Dion Backend] Connected to PHP + MySQL Gateway:', health);
  ...
  const addPayment = async (paymentData) => {
    ...
    try {
      ...
      if (res && res.data) {
        setPayments(prev => prev.map(p => p.id === newPayment.id ? { ...p, ...res.data } : p));
      }
    } catch (e) {
      // Fallback
    }
  ...
  const addBlock = (blockData) => {
    ...
    const newBlock = {
      id: `BLK-${nextLetter}`,
      name: blockData.name || `Block ${nextLetter}`,
      sector: blockData.sector || 'Sector 2',
      wings: blockData.wings || 'Wings A1 – A4',
      totalHouses: Number(blockData.totalHouses) || 40,
      occupiedHouses: 0,
      vacantHouses: Number(blockData.totalHouses) || 40,
      residentsCount: 0,
      occupancyRate: 0,
      status: 'Operational',
      securityOfficer: 'Unassigned'
    };
    setBlocks(prev => [...prev, newBlock]);
    recordAudit('BLOCK_CREATED', `Created new block ${newBlock.name}`);
    addToast(`Block ${newBlock.name} registered.`, 'success');
    return newBlock;
  };
  ...
  const updateBlock = async (blockId, blockData) => {
    ...
    try {
      const res = await api.updateBlock(blockId, blockData);
      if (res && res.data) {
  ...
  const updateHouse = async (houseId, houseData) => {
    ...
    try {
      const res = await api.updateHouse(houseId, houseData);
      if (res && res.data) {
        setHouses(prev => prev.map(h => (h.id === houseId || h.unitNumber === houseId) ? { ...h, ...res.data } : h));
      }
    } catch (e) {}
  };
  ...
  const resetToDefaults = async () => {
    try {
      ...
      Object.keys(localStorage).forEach(key => {
        if (key.startsWith(DATA_STORAGE_PREFIX)) {
          localStorage.removeItem(key);
        }
      });
      // Resync directly from backend MySQL
      const health = await api.getHealth();
      if (health && health.status === 'online') {
        const [uRes, bRes, hRes, rRes, fRes, eRes, vRes, gRes, tRes, pRes, sRes, aRes] = await Promise.allSettled([
          api.getUsers(),
        ...
        if (sRes.status === 'fulfilled' && sRes.value?.data) setSettings(sRes.value.data);
        if (aRes.status === 'fulfilled' && aRes.value?.data) setAuditLogs(aRes.value.data);
        addToast('State resynchronized with live database.', 'info');
      }
    } catch (e) {
  ...
  const value = {
    ...
    updateHouse,
    triggerPanicAlert,
    triggerPanicFlag: triggerPanicAlert,
    updateSettings,
    createResident,
    createFamilyMember,
    enrolEmployee,
